...
df_v2 = pd.read_csv(f'data/v2_{subsample}.txt')
...
class ForwardModel(jft.Model):
    ...
    def __call__(self, x):
        ...
        def complicated_function(rho_s, sigma_s, rho_dm, cf):
            ...
            return integral * norm/jnp.sum(integral), surface_density_calc, jnp.log(sig2), jnp.sum(params[:,0])
        dfo, sd, logsig2, rho = complicated_function(rs, ss, rdm, cf)
        return jft.Vector({'dfo': dfo, 'sd': sd, 'logsig2': logsig2, 'rho': rho})

fwd = ForwardModel()
R_dfo = jft.Model(lambda x: x['dfo'], domain=fwd.target)
R_sd = jft.Model(lambda x: x['sd'], domain=fwd.target)
R_sig2 = jft.Model(lambda x: x['logsig2'], domain=fwd.target)
...
lh_sig2 = jft.Gaussian(jnp.log(v2), lambda x: 1/elogv2**2 * x).amend(R_sig2)
...
data_cf = {
    ...
    "Error Data": [elogv2*jnp.ones_like(v2)],
}
