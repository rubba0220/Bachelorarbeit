...
data_roh = pd.read_csv('data_roh.csv', header=None)
data_sigma = pd.read_csv('data_sigma.csv', header=None)
...
def histogram2(data, titel, label, name='', save=False):
    ...
    plt.title(titel)
    n, bins, patches = axarray[0].hist(data,bins=jnp.arange(-3, 3, 0.25))
    ...
    ind = jnp.where(n>0)
    popt, perr = curve_fit(gauß, jnp.arange(-3,3,0.25)[ind]+0.125, n[ind]/A, p0=[0, 1.], sigma=jnp.sqrt(n[ind])/A, absolute_sigma=True)
    xi = n[ind]
    Xi = A * gauß(jnp.arange(-3,3,0.25)[ind]+0.125, *popt)
    ...
    axarray[1].errorbar(jnp.arange(-3, 3, 0.25)[ind]+0.125, xi-Xi, yerr=np.sqrt(Xi), color='red', fmt='o', markeredgecolor='red')
